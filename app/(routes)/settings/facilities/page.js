...
const FacilitiesPage = () => {
  ...
  const filteredFacilities = facilities.filter((user) => {
    ...
    return (
      ...
      user.datimCode.toLowerCase().includes(lowercaseSearchTerm) ||
      user.lga.lgaName.toLowerCase().includes(lowercaseSearchTerm) ||
      user.state.stateName.toLowerCase().includes(lowercaseSearchTerm)
    );
  });
                    ...
                    filteredFacilities.map((facilities, index) => (
                      <tr key={facilities.id}>
                        ...
                        <td className="text-center">
                          <i onClick={() => handleEdit(facilities.id)} className=" btn mx-1 btn-outline-primary btn-sm fa fa-pencil"></i>
