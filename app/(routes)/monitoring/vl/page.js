...
function VL () {
  ...
  let countImoVlSampleCollection = 0;
   ...
   let countImoVlUnSuppressed = 0;
     ...
     // Variables for VL Low Level Viremia
     const [llv, setllv] = useState(0);
     const [llvAbia, setllvAbia] = useState(0);
     const [llvEnugu, setllvEnugu] = useState(0);
     const [llvImo, setllvImo] = useState(0);
     let countAbiallv = 0;
     let countEnugullv = 0;
     let countImollv = 0;

     
     // Variables for VL Untectable
     const [undetectable, setUndetectable] = useState(0);
     const [undetectableAbia, setUndetectableAbia] = useState(0);
     const [undetectableEnugu, setUndetectableEnugu] = useState(0);
     const [undetectableImo, setUndetectableImo] = useState(0);
     let countAbiaUndetectable = 0;
     let countEnuguUndetectable = 0;
     let countImoUndetectable = 0;
     ...
     try {
      ...
      if(filter_lgas.length > 0 && selectedFacility == 0){
        ...
        setFacilityTotal(selectedFacility.length)
          ...
          //EndPoint for VL Low Level Viremia
          axios.post(bURL + "vlchart/llv", vlDataType).then((response)=>{
        
            //console.log(response.data);
            setllv(response.data.sumTotal);
            //console.log(response.data.charts);
            const vlChart = response.data.charts;
            vlChart.filter((data) => data.state === 'Abia').map(data => {countAbiallv = countAbiallv + data.total_aggregate;})
            vlChart.filter((data1) => data1.state === 'Enugu').map(data1 => {countEnugullv = countEnugullv + data1.total_aggregate;})
            vlChart.filter((data2) => data2.state === 'Imo').map(data2 => {countImollv = countImollv + data2.total_aggregate;})
         //Set the count values for each state
           setllvAbia(countAbiallv);
           setllvEnugu(countEnugullv);
           setllvImo(countImollv);
            
        });

         //EndPoint for VL Undetectable
         axios.post(bURL + "vlchart/undetectable", vlDataType).then((response)=>{
      
          //console.log(response.data);
          setUndetectable(response.data.sumTotal);
          //console.log(response.data.charts);
          const vlChart = response.data.charts;
          vlChart.filter((data) => data.state === 'Abia').map(data => {countAbiaUndetectable = countAbiaUndetectable + data.total_aggregate;})
          vlChart.filter((data1) => data1.state === 'Enugu').map(data1 => {countEnuguUndetectable = countEnuguUndetectable + data1.total_aggregate;})
          vlChart.filter((data2) => data2.state === 'Imo').map(data2 => {countImoUndetectable = countImoUndetectable + data2.total_aggregate;})
       //Set the count values for each state
         setUndetectableAbia(countAbiaUndetectable);
         setUndetectableEnugu(countEnuguUndetectable);
         setUndetectableImo(countImoUndetectable);
...
return (
  <div className="container mt-3">
    <div className="row">
      <div className="col-md-12 text-center p-3"><h3>Viral Load Monitoring</h3></div>
    </div>
    {/* LineBar View */}
    <div className="row">
...
<LocationCard 
        ...
        <div className="row">
          <GraphCard indicator = "VL RESULT CLASSIFICATION " description=""  chartType={<DonutChart unsuppressed = {vlUnSuppressed} llv ={llv} undetectable={undetectable}  chartTypeNumber={11}/>} />
